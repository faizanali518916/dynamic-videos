...
export const isVideoShownSegment = (
  segment: InfographicSegment,
): segment is VideoShownSegment => segment.videoShown === true;
...
export const getLayoutItemCount = (segment: AnimationSegment): number => {
  ...
  const requestedItems =
    providedItems > 0 ? providedItems : spec.recommendedItems;

  return Math.round(clampNumber(requestedItems, spec.minItems, spec.maxItems));
};

export const getSegmentDurationSeconds = (
  segment: InfographicSegment,
): number => {
  if (isVideoShownSegment(segment)) {
    ...
    const duration = Number.isFinite(requestedDuration)
      ? requestedDuration
      : DEFAULT_VIDEO_SCENE_DURATION_SECONDS;
...
export const getSegmentDurationInFrames = (
  segment: InfographicSegment,
  fps = FPS,
): number => Math.round(getSegmentDurationSeconds(segment) * fps);

export const getTemplateDurationInFrames = (
  template: InfographicTemplate,
  fps = FPS,
): number => {
  const segmentFrames = template.segments.reduce(
