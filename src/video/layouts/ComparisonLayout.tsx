...
export const ComparisonLayout = ({ accent, frame, segment, theme }: LayoutProps) => {
  ...
  return (
    <div style={{ display: "grid", gap: 30, gridTemplateColumns: "1fr 1fr", width: "100%" }}>
      {columns.map((column, columnIndex) => (
        <GlowCard
          ...
          theme={theme}
          style={{ padding: "32px 28px", transform: `rotate(${columnIndex === 0 ? -1 : 1}deg)` }}
        >
              ...
              >
                {columnIndex === 1 ? (
                  <Arrow
                    accent={column.color}
                    opacity={0.7}
                    style={{ position: "absolute", right: 18, top: 18, transform: "scale(0.55)" }}
                  />
                ) : null}
                {item}
              </div>
